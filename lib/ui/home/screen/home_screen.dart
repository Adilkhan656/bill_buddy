...
import 'package:bill_buddy/ui/home/widget/budget_alert.dart';
import 'package:bill_buddy/ui/home/widget/home_filter.dart';
import 'package:bill_buddy/ui/home/widget/transection_list.dart';
import 'package:flutter/material.dart';
import 'package:intl/intl.dart';
import 'package:lottie/lottie.dart';
import 'package:provider/provider.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        child: Lottie.asset('assets/lottie/Scanner_Animation.json', width: 56, height: 56),
      ),
      bottomNavigationBar: BottomAppBar(
        shape: const CircularNotchedRectangle(),
        notchMargin: 10,
      ...
      appBar: AppBar(
        ...
        title: user == null
            ...
            : FutureBuilder<UserProfile?>(
                ...
                builder: (_, snap) {
                  final name = snap.data?.name ?? user.displayName ?? "User";
                  return Text("Hello, ${name.split(" ").first} 👋", style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold));
                },
              ),
        actions: [
          Padding(
        ...
        builder: (context, snapshot) {
          ...
          final listExpenses = allExpenses.where((e) {
            if (_searchQuery.isNotEmpty) {
              ...
              final dateStr = DateFormat('dd MMM yyyy').format(e.date).toLowerCase();
              if (!e.merchant.toLowerCase().contains(searchLower) &&
                  !e.category.toLowerCase().contains(searchLower) &&
                  !dateStr.contains(searchLower)) {
                return false;
              }
            }
            if (_selectedCategoryFilter != "All" && e.category != _selectedCategoryFilter) return false;
            if (_listMonthFilter != null && (e.date.year != _listMonthFilter!.year || e.date.month != _listMonthFilter!.month)) return false;
            return true;
          }).toList();

          return CustomScrollView(
            slivers: [
              // 1. Budget Alerts (New Clean UI)
              SliverToBoxAdapter(
                child: Stack(
                  alignment: Alignment.topCenter,
                  children: [
                    // 1. THE DASHBOARD (Bottom Layer - No extra padding)
                    DashboardCard(
                      viewModel: _viewModel,
                      selectedDate: _dashboardMonth,
                      onDateChanged: (date) => setState(() => _dashboardMonth = date),
                    ),

                    // 2. THE ALERTS (Top Layer - Floating)
                    // Added a small top margin so it doesn't touch the very edge
                    Positioned(
                      top: 0, 
                      left: 0,
                      right: 0,
                      child: BudgetAlerts(allExpenses: allExpenses),
                    ),
                  ],
                ),
              ),
              // 3. Filters & Search (New File)
              SliverToBoxAdapter(
                child: HomeFilters(
                  selectedMonth: _listMonthFilter,
                  selectedCategory: _selectedCategoryFilter,
                  onSearchChanged: (val) => setState(() => _searchQuery = val.trim().toLowerCase()),
                  onMonthChanged: (val) => setState(() => _listMonthFilter = val),
                  onCategoryChanged: (val) => setState(() => _selectedCategoryFilter = val),
                ),
              ),

              // 4. Transaction List (New File)
              TransactionList(expenses: listExpenses, searchQuery: _searchQuery),
